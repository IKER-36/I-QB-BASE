...
AddEventHandler('playerDropped', function(reason) 
	local src = source
	print("Dropeado: "..GetPlayerName(src))
...
local function OnPlayerConnecting(name, setKickReason, deferrals)
    ...
    deferrals.update(string.format("Bienvenido! %s. Estamos Validando su licencia de Rockstar", name))
    ...
    deferrals.update(string.format("Hola! %s. un momento, estamos checkeando si fuiste baneado.", name))
    ...
    deferrals.update(string.format("Bienvenido %s a {Server Name}.", name))

    if not license then
        deferrals.done('No se encontró ninguna licencia de Rockstar válida')
    elseif isBanned then
	    deferrals.done(Reason)
    elseif isLicenseAlreadyInUse then
        deferrals.done('Licencia duplicada de Rockstar encontrada')
    else
        deferrals.done()
        Wait(1000)
...
AddEventHandler('QBCore:server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(source, "admin") or QBCore.Functions.HasPermission(source, "god") then 
        local reason = reason ~= nil and reason or "Sin razón especificada..."
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "Ahora estás fuera de servicio!")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "Ahora estas de servicio!")
	end
