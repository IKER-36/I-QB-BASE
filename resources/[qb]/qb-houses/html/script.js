...
$('document').ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.type == "buyOption") {
            ...
            $(".decorate-confirm").find("p").html("¿Está seguro de que desea comprar este objeto por $"+selectedObjectData.price+"?");
        }
            ...
            if (selectedObjectData != null) {
                ...
                $.post('https://qb-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    ...
                    $.each(myObjects, function(i, object){
                        var elem = '<div class="decorate-item" id="myobject-'+i+'" data-type="myObject"><span id="decorate-item-name"><b>Objeto: </b>'+object.hashname+'</span><span id="decorate-item-category"><strong>Precio: </strong><span id="item-price" style="color: green;">TUYO</span></span></div>';
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $.each(houseCategorys[selectedCategory].items, function(i, item){
            var elem = '<div class="decorate-item" id="object-'+i+'" data-type="newObject"><span id="decorate-item-name"><b>Objeto: </b>'+(item.label).charAt(0).toUpperCase() +''+(item.label).substr(1).toLowerCase()+'</span><span id="decorate-item-category"><strong>Precio: </strong><span id="item-price" style="color: green;">$'+item.price+'</span></span></div>';
            $('.decorate-items').append(elem);
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        ...
        if (selectedObject == this) {
            ...
            $(".decorate-footer-buttons").html("");
            var elem = '<div class="footer-btn" id="remove-owned-obj"><p>Retirar</p></div>';
            $(".decorate-footer-buttons").append(elem);
            $(".decorate-footer-buttons").fadeIn(150);
        }
    }
