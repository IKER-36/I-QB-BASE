                ...
                if dealerDist <= 1.5 and not isHealingPerson then
                    if not interacting then
                        if not dealerIsHome then
                            DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '[E] Golpear')
                        ...
                        elseif dealerIsHome then
                            if dealer["name"] == "Mystery man" then
                                DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '[E] Comprar / [G] Ayuda a tu colega ($5000)')
                            else
                                DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '[E] Comprar / [G] Comienza una misión')
                            end
                            if IsControlJustPressed(0, 38) then
                                buyDealerStuff()
                            end
                            ...
                            if IsControlJustPressed(0, 47) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        QBCore.Functions.Progressbar("hospital_revive", "Pon a la persona de nuevo en pie..", 5000, false, true, {
                                            ...
                                            QBCore.Functions.Notify("Has ayudado a una persona!")
                                    ...
                                    if waitingDelivery == nil then
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "Estos son los productos, me mantendré en contacto por correo electrónico")
                                        requestDelivery()
                                        interacting = false
                                        dealerIsHome = false
                                    else
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "error", 'Aún necesitas completar una entrega, ¿A qué estás esperando?!')
                                    end
                                end
                            end
                        end
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        if Config.Dealers[currentDealer]["name"] == "Mystery man" then
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Hola mi niño, que puedo hacer por ti')
        elseif Config.Dealers[currentDealer]["name"] == "Fred" then
            dealerIsHome = false
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Lamentablemente ya no hago negocios contigo ... Deberías haberme tratado mejor')
        else
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Yo '..myData.charinfo.firstname..', Qué puedo hacer por ti?')
        end
        -- knockTimeout()
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.2)
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        QBCore.Functions.Notify('Parece que no hay nadie en casa..', 'error', 3500)
    end
end
...
function requestDelivery()
    ...
    SetTimeout(2000, function()
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..QBCore.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> Sé puntual!!",
            button = {
                enabled = true,
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    QBCore.Functions.Notify('La ruta a la ubicación de entrega se ha establecido en su mapa.', 'success');
end
...
AddEventHandler('qb-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        ...
        QBCore.Functions.Notify('Aún tienes una entrega abierta...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..QBCore.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' entregar.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        QBCore.Functions.Progressbar("work_dropbox", "Entrega de productos..", 3500, false, true, {
...
AddEventHandler('qb-drugs:client:robberyCall', function(msg, streetLabel, coords)
    ...
    AddTextComponentString("911: Tráfico de drogas")
...
AddEventHandler('qb-drugs:client:sendDeliveryMail', function(type, deliveryData)
    if type == 'perfect' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            subject = "Delivery",
            message = "Hiciste un buen trabajo, espero verte de nuevo ;)<br><br>Groeten, "..Config.Dealers[deliveryData["dealer"]]["name"]
        })
    elseif type == 'bad' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Delivery",
            message = "He recibido quejas sobre tu entrega, no dejes que esto vuelva a suceder...."
        })
    elseif type == 'late' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Delivery",
            message = "No llegaste a tiempo. ¿Tenías cosas más importantes que hacer que los negocios?"
        })
    end
end)
...
AddEventHandler('qb-drugs:client:GotoDealer', function(DealerData)
    ...
    QBCore.Functions.Notify('Has sido teletransportado : '.. DealerData["name"] .. ' Buena suerte!', 'success')
end)
