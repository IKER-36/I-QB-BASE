                ...
                if MainDistance < 2 then
                    ...
                    if TaskData ~= nil then
                        if not BuilderData.ShowDetails then
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '[E] Vista de detalles')
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z + 0.2, 'Ejercicios: '..TaskData.completed..' / '..TaskData.total)
                        else
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '[E] Vista de detalles')
                            for k, v in pairs(Config.Projects[Config.CurrentProject].ProjectLocations["tasks"]) do
                                if v.completed then
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z + OffsetZ, v.label..': Terminado')
                                else
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z + OffsetZ, v.label..': Sin terminar')
                                end
                                OffsetZ = OffsetZ + 0.2
                            end
                        end
                        ...
                        if TaskData.completed == TaskData.total then
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.2, '[G] Finalizar proyecto')
                ...
                if not v.completed or not v.IsBusy then
                    ...
                    if TaskDistance < 10 then
                        ...
                        if TaskDistance < 1.5 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.25, '[E] Tarea completada')
                            if IsControlJustPressed(0, 38) then
